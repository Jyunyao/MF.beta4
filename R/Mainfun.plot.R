    ...
    if(is.null(by_group)){
      ...
      plot_output <- ggplot(data = output, aes(x = Species.diversity, y = qMF))+
        ...
        geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=paste0("Slope = ", round(estimate, 4)), col=group),
                  hjust= -0.1, vjust= 2, size=3.2, key_glyph = draw_key_path)+
        ...
        theme(legend.position = "bottom", legend.box = "vertical", legend.margin=margin(-6,-6,0,-6),legend.text = element_text(size=12, margin = margin(r = 1, unit = 'cm')),
              strip.text = element_text(size=15),axis.text = element_text(size=8),axis.title=element_text(size=18),
      ...
      if(text == "Slope"){
        plot_output <- plot_output +
          geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=3.2,key_glyph = draw_key_path)+
        ...
        plot_output <- plot_output +
          geom_text(data = lm_text, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=3.2,key_glyph = draw_key_path, parse = T,col="red")+
          scale_colour_manual(values = col_manual) +
          theme_bw() +
