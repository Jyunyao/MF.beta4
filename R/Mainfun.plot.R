  ...
  # For MF_single output
    ...
    if(is.null(by_group)){
      ...
      plot_output <- ggplot(data = output, aes(x = Species.diversity, y = qMF))+
        ...
        theme_bw() +
        theme(legend.position = "bottom", legend.box = "vertical", legend.margin=margin(-6,0,0,0), legend.title = element_blank(),text = element_text(size=20,face = "bold"))+
        guides(
      ...
      if(text == "Slope"){
        plot_output <- plot_output +
          ...
          theme(legend.position = "bottom", legend.box = "vertical", legend.margin=margin(-6,0,0,0),text = element_text(size=20,face = "bold"))+
          labs(x = "Species diversity", y = "Multifunctionality")
      }
      else{
