    ...
    if(is.null(by_group)){
      ...
      plot_output <- ggplot(data = output, aes(x = Species.diversity, y = qMF))+
        ...
        geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=paste0("Slope = ", round(estimate, 4)), col=group),
                  hjust= -0.1, vjust= 2, size=4, key_glyph = draw_key_path)+
      ...
      if(text == "Slope"){
        plot_output <- plot_output +
          geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=4,key_glyph = draw_key_path)+
        ...
        plot_output <- plot_output +
          geom_text(data = lm_text, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=4,key_glyph = draw_key_path, parse = T,col="red")+
          scale_colour_manual(values = col_manual) +
          theme_bw() +
      ...
      scale_plot <- function(out, xlab, ylab, digit=2, h_j=-1.5){
        if(is.null(by_group)){
          ...
          plot_output <- ggplot(data = out, aes(x = Species.diversity, y = qMF))+
            ...
            geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=paste0("Slope = ", round(estimate, 4)), col=group),
                      hjust= -0.1, vjust= 2, size=3.3, key_glyph = draw_key_path)+
            scale_color_manual(values = "red")+
            theme_bw() +
          ...
          if(text == "Slope"){
            plot_output <- plot_output +
              geom_text(data = lm_data, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=3.3,key_glyph = draw_key_path)+
              scale_colour_manual(values = col_manual) +
              theme_bw() +
              guides(linetype = guide_legend(title = "",order = 1,override.aes = list(col = "#000000",size=0.6)),
                     col = guide_legend(title = by_group))+
            ...
            plot_output <- plot_output +
              geom_text(data = lm_text, aes(x = -Inf, y = Inf, label=Label, hjust= h, vjust= v), size=3.3,key_glyph = draw_key_path, parse = T,col="red")+
              scale_colour_manual(values = col_manual) +
              theme_bw() +
              guides(linetype = guide_legend(title = "",order = 1,override.aes = list(col = "red",size=0.6)),
                     col = guide_legend(title = by_group))+
