...
NOTE 1: Because the total number of pair plots is huge for the whole data, it is very time consuming to obtain the BEF graphical relationships from running the above code for alpha/beta/gamma scales. Users can simply select two countries (with 10 plots in each country) to promptly view the graphical results; see the package manual for examples. 

NOTE 2: The BEF relationships shown in the above two figures (uncorrected and corrected for correlations) are slightly different from those in Chao et al. (2023). This is because in Chao et al.'s paper, data from Finland are excluded (due to sparse data), whereas all data are included in the above BEF analysis.
